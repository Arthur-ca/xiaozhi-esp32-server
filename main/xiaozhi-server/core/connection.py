...
class ConnectionHandler:
    ...
    def chat(self, query, tool_call=False, depth=0):
        ...
        # === 新增：并发/重入保护，避免上一轮流未结束时再次借用同一生成器 ===
        # 若上一轮还在进行，先请求安全中断并等待其收尾（最多 2s）
        if not self.llm_finish_task:
            try:
                self.logger.bind(tag=TAG).warning("上一轮 LLM 流尚未结束，正在安全中断并切换到新的请求")
                self.client_abort = True
                import time as _time
                t0 = _time.time()
                # chat() 是同步函数，这里用短睡眠轮询（不会阻塞事件循环，因为 chat 通常在工作线程中执行）
                while not self.llm_finish_task and _time.time() - t0 < 2.0:
                    _time.sleep(0.01)
            finally:
                # 确保清理 abort 标记，为本轮请求让路
                self.client_abort = False
